...
const DEFAULT_FILE_NAME = 'weather_icons.dart'
...
async function scrapeIconData() {
  ...
  const renamed = filtered.icons.map(icon => ({
    ...icon,
    title: icon.title.replace('wi-', ''),
  }))
...
function generateIcon(title, code) {
  const icon = `static const IconData ${title} = const _Icon(0x${code});`
